...
# Define the parameters to sort by reactions
params = {
    "state": "open",  # You can change this to "closed" if you want closed issues
    "sort": "reactions",  # Sort by reactions
    "direction": "desc",  # Sort in descending order (most reactions first)
}
...
# Make the GET request to fetch the sorted issues
...
if response.status_code == 200:
    ...
    for issue in issues:
        print(f"Issue #{issue['number']}: {issue['title']} ({issue['reactions']['total']} reactions)")
else:
